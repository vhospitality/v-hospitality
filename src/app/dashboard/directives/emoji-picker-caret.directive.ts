...
export class EmojiPickerCaretDirective implements OnInit, OnDestroy {
  @Output('emojiPickerCaretEmitter') caretEmitter =
    new EventEmitter<CaretEvent>();
  ...
  get doc() {
    if (!this._doc) {
      this._doc =
        ...
        this._el.nativeElement.document ||
        document;
    }
  ...
  ngOnInit() {
    if (
      !this._el.nativeElement.getAttribute('contenteditable') &&
      this._el.nativeElement.tagName.toLowerCase() !== 'input'
    ) {
      throw new Error(
        '(emojiPickerPositionEmitter) should only work on contenteditable enabled or input elements'
      );
    }
  }
  ...
  updateCaretPosition() {
    const cEvent = CaretEvent.generateCaretEvent(
      this.win,
      this.doc,
  ...
  updateCaretDueMutation() {
    ...
    let textMovement =
      cEvent.textContent.length - this._lastCaretEvent.textContent.length;
